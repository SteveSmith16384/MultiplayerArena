...
public class JoystickCamera2 extends FlyByCamera implements IInputDevice, RawInputListener {
	...
	private static final float LOOK_UD_ADJ = MultiplayerArenaMain.properties.GetGamepadUpDownAdjust();// .4f;//.75f;//.5f;
	private static final float MOVE_SPEED = MultiplayerArenaMain.properties.GetGamepadMoveSpeed();// 5;
	private static final float DEADZONE = MultiplayerArenaMain.properties.GetGamepadDeadZone();// 0.0015f;
	private static final float TURN_SPEED = MultiplayerArenaMain.properties.GetGamepadTurnSpeed();// 100f;//150f;
